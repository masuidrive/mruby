...
void mrb_debug_gem_init(mrb_state* mrb) {
  ...
  saved_hooks[mrb_count - 1].hook_mrb_read_irep = mrb->hook_mrb_read_irep;
...
void mrb_debug_gem_final(mrb_state* mrb) {
#ifdef ENABLE_DEBUGGER
  int i;
  for(i = 0; i < mrb_count; ++i) {
    if(saved_hooks[i].mrb == mrb) {
      mrb->hook_mrb_read_irep = saved_hooks[i].hook_mrb_read_irep;
